...
- 🖱️ **鼠标连点** — 固定间隔自动点击，可跟随光标或锁定坐标
- ⌨️ **键盘连按** — 定时点按 / 按住两种模式；「点击按键」可捕获单个键或**多个同时按下的键**（如 `Shift+A`）
- ⏺️ **录制回放** — 全局低级钩子录制真实鼠标/键盘操作，自动合并冗余动作（连续移动合并为"移动到终点"、短按合并为单击/按键），可编辑、保存、按倍速循环回放
- ⏱️ **运行限制三选项** — 连点 / 连按 / 回放 三个页面同一位置的运行限制控件：**指定次数 / 无限循环 / 运行时长(时·分·秒) 或 运行到指定时刻**；「时长」与「截止时刻」双向同步（设 10 分钟即显示本地时间 +10 分钟）
- 🔥 **全局热键** — 5 个功能开关全部可自定义，支持任意组合键：多键组合（如 `Ctrl+Q+W`）、鼠标侧键、媒体键
- 🔊 **按键音效** — 任意单键/组合键绑定 wav/mp3 音效，按下即播，新键覆盖旧音效（覆盖式播放）；**支持命名场景**（每个场景 = `Sounds` 下的一个子文件夹 + 独立的一套绑定，一键整体切换）
...
「音效」页可把任意单键或组合键绑定到 wav/mp3 音效文件，按下即播放，新按下的键会打断正在播放的音效。**场景**：每个场景是 `Sounds` 下的一个子文件夹（如 `Sounds\绘画\`）加独立的一套绑定，用「场景」下拉框切换、「新建场景」创建，添加绑定时音效文件自动复制进当前场景文件夹。支持总开关、**全局音效音量**（总音量）与该键的相对音量，实际响度 = 两者相乘（全局 0 = 全部静音）。程序启动时会自动预热音频设备，不需要先「试听」一次。
...
| [src/AutoClicker.cs](src/AutoClicker.cs) / [src/KeyboardSpammer.cs](src/KeyboardSpammer.cs) | 鼠标连点 / 键盘连按引擎（连按支持多键同时按下） |
| [src/RunLimit.cs](src/RunLimit.cs) | 三个功能共用的运行限制控件（次数 / 无限 / 时长·截止时刻双向同步） |
